...
(function() {
  ...
  // ==========================================
  // PANEL 1: TELEGRAM WORKERS & KEYS (LIVE MASI SYNC)
  // ==========================================
  function renderGuysPanel(container) {
    const liveKeyMap = getLiveWorkerKeyMap();
    const search = (state.searchQuery || '').toLowerCase().trim();

    // Prepare enriched Telegram workers list with live data from masi
    const enrichedList = telegramWorkersMapping.map((tgUser, idx) => {
      let userTotalCompleted = 0;
      let userTodayDone = 0;
      let userTotalRate = 0;
      let onlineKeysCount = 0;

      const keysEnriched = tgUser.keys.map(k => {
        const live = liveKeyMap.get(k.key.toUpperCase()) || liveKeyMap.get(k.name.toLowerCase()) || {};
        const completedCount = Number(live.completedOrders !== undefined ? live.completedOrders : (live.completedCount || 0));
        const successRate = Number(live.successRate || 0);
        const todayDone = Number(live.todayDone || 0);
        const online = Boolean(live.online);

        userTotalCompleted += completedCount;
        userTodayDone += todayDone;
        userTotalRate += successRate;
        if (online) onlineKeysCount++;
      ...
      const avgSuccessRate = keysEnriched.length > 0 ? (userTotalRate / keysEnriched.length).toFixed(1) : '0';
      ...
      return {
        ...
        keys: keysEnriched,
        totalCompleted: userTotalCompleted,
        todayDone: userTodayDone,
        avgSuccessRate,
        onlineKeysCount,
        hasOnline: onlineKeysCount > 0
      };
    });

    // Filter by search
    const filtered = enrichedList.filter(u => {
      if (!search) return true;
      const matchUser = u.username.toLowerCase().includes(search) || u.displayName.toLowerCase().includes(search);
      const matchKey = u.keys.some(k => k.name.toLowerCase().includes(search) || k.key.toLowerCase().includes(search));
      return matchUser || matchKey;
    });
    ...
    let html = `
      <div class="space-y-6">
        <!-- Top Hero Banner with Live Sync & Copy Notice -->
        <div class="bg-gradient-to-r from-indigo-700 via-sky-700 to-emerald-700 rounded-2xl shadow-lg p-5 sm:p-6 text-white flex flex-wrap items-center justify-between gap-4">
          <div>
            <div class="flex items-center gap-2 mb-1.5 flex-wrap">
              <span class="px-2.5 py-0.5 rounded-full text-xs font-black uppercase tracking-wider bg-amber-400 text-slate-900 shadow-sm">⚡ 1-Tap Copy Enabled</span>
              <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-emerald-500/80 text-white backdrop-blur-xs flex items-center gap-1.5">
                <span class="w-2 h-2 rounded-full bg-emerald-200 animate-ping"></span> Live masi.cc.cd Sync
              </span>
              <span class="px-2 py-0.5 rounded text-[11px] bg-white/20 text-white">Last Sync: ${lastSyncTimeStr}</span>
            </div>
            <h2 class="text-2xl sm:text-3xl font-black tracking-tight text-white flex items-center gap-2">
              <span>✈️</span> Telegram Workers & Keys Hub
            </h2>
            <p class="text-xs sm:text-sm text-sky-100 max-w-2xl mt-1 leading-relaxed">
              Every worker key is grouped under their Telegram username. Click or tap ANY key to instantly copy it. Live order counts, today's ok, and rates are fetched real-time directly from <b>masi.cc.cd</b>.
            </p>
          </div>

          <div class="flex items-center gap-2.5">
            <button onclick="fetchLiveMasiData(true)" id="btn-sync-live-top" class="px-4 py-2.5 rounded-xl text-xs font-extrabold bg-white text-indigo-900 hover:bg-indigo-50 active:scale-95 shadow-md transition flex items-center gap-2">
              <svg id="sync-spinner-icon" class="w-4 h-4 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path></svg>
              <span id="sync-btn-label">🔄 Fetch Live Data from masi.cc.cd</span>
            </button>
          </div>
        </div>

        <!-- ⚡ PROMINENT CALLOUT: ONE-TAP TO COPY KEY -->
        <div class="bg-gradient-to-r from-amber-50 via-indigo-50 to-emerald-50 border-2 border-indigo-400/90 rounded-2xl p-4 sm:p-5 shadow-sm flex flex-wrap items-center justify-between gap-4">
          <div class="flex items-center gap-3.5">
            <div class="w-12 h-12 rounded-2xl bg-indigo-600 text-white flex items-center justify-center font-black text-2xl shadow-md">
              ⚡
            </div>
            <div>
              <div class="flex items-center gap-2">
                <h3 class="text-base sm:text-lg font-black text-indigo-950 uppercase tracking-wide">
                  One-Tap to Copy Any Key
                </h3>
                <span class="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-black bg-indigo-600 text-white shadow-2xs">
                  Instant Copy
                </span>
              </div>
              <p class="text-xs sm:text-sm text-slate-700 mt-0.5">
                Simply <b>click or tap any key badge</b> below to instantly copy the exact key code (<code class="bg-white px-2 py-0.5 rounded border border-indigo-200 text-indigo-800 font-mono font-bold text-xs">WORKER-...</code>) directly to your clipboard!
              </p>
            </div>
          </div>
          <div class="text-xs text-indigo-900 font-bold bg-white/90 backdrop-blur-xs px-3.5 py-2 rounded-xl border border-indigo-200 shadow-2xs flex items-center gap-2">
            <span>🖱️ Click on Desktop</span>
            <span class="text-slate-300">|</span>
            <span>📱 Tap on Mobile</span>
          </div>
        </div>

        <!-- Workers List: Clean Cards for the 24 Telegram Users -->
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ${filtered.map(u => `
            <div class="bg-white rounded-2xl border border-slate-200 hover:border-indigo-300 shadow-xs hover:shadow-md transition p-4 sm:p-5 flex flex-col justify-between">
              <!-- Telegram User Header -->
              <div class="flex items-center justify-between gap-2 pb-3 border-b border-slate-100">
                <div class="flex items-center gap-2.5">
                  <div class="w-10 h-10 rounded-xl bg-gradient-to-tr from-sky-500 to-indigo-600 text-white flex items-center justify-center font-black text-sm shadow-xs">
                    ${u.rank}
                  </div>
                  <div>
                    <div class="flex items-center gap-1.5">
                      <span class="text-base font-extrabold text-slate-900">${escapeHtml(u.username)}</span>
                      ${u.username.startsWith('@') ? `
                        <button onclick="copyToClipboard('${escapeHtml(u.username)}', 'Telegram Handle')" title="Copy Telegram Username" class="text-slate-400 hover:text-sky-600 transition p-1">
                          <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg>
                        </button>
                      ` : ''}
                    </div>
                    <span class="text-xs text-slate-400">${escapeHtml(u.displayName)}</span>
                  </div>
                </div>

                <!-- Combined Live Stats Badge -->
                <div class="text-right">
                  <div class="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-200 text-xs font-bold">
                    <span>${u.totalCompleted} Completed ✓</span>
                  </div>
                  <div class="text-[11px] text-slate-400 mt-0.5">
                    ${u.keys.length} Key${u.keys.length > 1 ? 's' : ''} • Avg ${u.avgSuccessRate}%
                  </div>
                </div>
              </div>

              <!-- Keys Under That Telegram Name (One-Tap Copy Enabled) -->
              <div class="mt-3.5 space-y-2">
                <div class="text-[11px] font-black text-slate-400 uppercase tracking-wider flex items-center justify-between">
                  <span>Assigned Keys (⚡ Tap to Copy):</span>
                  <span class="text-indigo-600 font-bold">${u.keys.length} Total</span>
                </div>

                ${u.keys.map(k => `
                  <div 
                    onclick="copyToClipboard('${escapeHtml(k.key)}', 'Worker Key: ${escapeHtml(k.name)}')" 
                    title="⚡ Click / Tap to copy key: ${escapeHtml(k.key)}"
                    class="group p-3 rounded-xl bg-slate-50/80 hover:bg-indigo-50/70 border border-slate-200/90 hover:border-indigo-400 shadow-2xs hover:shadow-sm cursor-pointer transition flex items-center justify-between gap-3 active:scale-[0.99] select-none"
                  >
                    <div class="flex items-center gap-2.5 min-w-0">
                      <div class="w-7 h-7 rounded-lg bg-white border border-slate-200 group-hover:border-indigo-300 text-indigo-600 flex items-center justify-center font-bold text-xs flex-shrink-0 shadow-2xs">
                        🔑
                      </div>
                      <div class="min-w-0">
                        <div class="flex items-center gap-1.5">
                          <span class="text-xs font-black text-slate-800 group-hover:text-indigo-900 truncate">${escapeHtml(k.name)}</span>
                        ...
                        <div class="font-mono text-[11px] font-semibold text-slate-500 group-hover:text-indigo-800 tracking-tight truncate">
                          ${escapeHtml(k.key)}
                        </div>
                      </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2 flex-shrink-0">
                      <div class="text-right hidden sm:block">
                        <span class="text-xs font-extrabold text-emerald-600 block">${k.completedCount}✓</span>
                        <span class="text-[10px] text-slate-400 block">${k.successRate}% rate</span>
                      </div>

                      <button 
                        onclick="event.stopPropagation(); copyToClipboard('${escapeHtml(k.key)}', 'Worker Key: ${escapeHtml(k.name)}')" 
                        title="One-Tap Copy" 
                        class="px-2.5 py-1.5 rounded-lg bg-white group-hover:bg-indigo-600 text-indigo-700 group-hover:text-white font-bold text-xs border border-indigo-200 group-hover:border-transparent transition flex items-center gap-1 shadow-2xs active:scale-95"
                      >
                        <span>⚡ Copy</span>
