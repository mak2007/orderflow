...
(function() {
        ...
        <div class="bg-white rounded-xl shadow-sm border ${isSettled ? 'paid-card-settled' : 'border-slate-200'} hover:border-indigo-300 transition-all overflow-hidden flex flex-col justify-between relative">
          ...
          <div class="p-3 ${isSettled ? 'bg-emerald-50/40 border-emerald-200' : 'bg-slate-50 border-slate-200'} border-t flex items-center justify-between gap-2 flex-wrap">
            ...
            <div class="flex items-center gap-1.5 flex-wrap">
              ${guy.unpaidAmount > 0 ? `
                <button onclick="markGuyPaid('${escapeHtml(guy.id)}')" title="Mark paid and notify via Telegram" class="px-3 py-1.5 rounded-lg text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white shadow-xs transition flex items-center gap-1">
                  <span>✓</span> Pay ₹${guy.unpaidAmount.toFixed(2)} ${isLinked ? '📲' : ''}
                </button>
              ` : `
                ...
                ${guy.completedOrders > 0 ? `
                  <button onclick="unmarkGuyPaid('${escapeHtml(guy.id)}')" title="Reset / Mark Unpaid" class="px-2 py-1 text-xs font-semibold text-rose-600 bg-rose-50 hover:bg-rose-100 border border-rose-200 rounded-md transition flex items-center gap-1">
          ...
          <tr class="hover:bg-slate-50/80 transition ${isSettled ? 'paid-row-settled' : ''}">
            ...
            <td class="py-3 px-4 text-right">
              <div class="inline-flex items-center gap-1.5 justify-end">
                <button onclick="openSendMsgModal('${escapeHtml(g.keys[0]?.id || '')}', '${escapeHtml(g.displayName || '')}', '${escapeHtml(g.telegramUsername || '')}')" title="Send direct Telegram message" class="p-1.5 rounded-lg text-xs font-bold text-sky-700 bg-sky-50 hover:bg-sky-100 border border-sky-200 transition">
                  💬
                </button>
                ${hasDue ? `
                ...
                ` : `
                  ...
                  ${g.completedOrders > 0 ? `
                    <button onclick="unmarkGuyPaid('${escapeHtml(g.id)}')" title="Reset / Mark Unpaid" class="px-2 py-1 text-xs font-semibold text-rose-600 bg-rose-50 hover:bg-rose-100 border border-rose-200 rounded-md transition">
                      ↩ Unpaid
                    </button>
                  ` : ''}
                `}
  ...
  window.unmarkGuyPaid = async function(guyId) {
    ...
    guy.keys.forEach(k => {
      ...
      k.lastPaidAt = null;
      if (state.isApiOnline) {
        fetch(`${API_BASE}/workers/${k.id}/mark-unpaid`, { method: 'POST' }).catch(() => {});
      }
    });

    saveToLocalStorage();
    render();
    showToast(`↩ Marked ${guy.displayName} as UNPAID (₹${guy.totalEarned.toFixed(2)} due)`, 'info');
  };
  ...
  // Direct 1-click live sync from laptop dashboard
  window.syncMasiLiveDirect = async function() {
    ...
    try {
      showToast('Fetching live stats from masi.cc.cd...', 'info');
      ...
      if (data.success && Array.isArray(data.workers)) {
        ...
        saveToLocalStorage();
        render();
        showToast(`✓ Live Synced: ${data.workers.length} workers updated with latest stats!`, 'success');
      } else {
      ...
      if (btn) {
        btn.disabled = false;
        btn.innerHTML = `<span>⚡</span> <span>Live Sync Masi</span>`;
      }
    }
  };

  // Send Direct Message to Worker's Telegram
  let activeMsgWorkerId = null;

  window.openSendMsgModal = function(workerId, workerName, tgUsername) {
    activeMsgWorkerId = workerId;
    ...
    if (desc) desc.textContent = `To: ${workerName} ${tgUsername ? `(${tgUsername})` : ''}`;
    if (input) {
      input.value = '';
    ...
    const hasTg = worker && (worker.telegramId || worker.telegramUsername);
    if (warning) {
      if (!hasTg) {
        warning.classList.remove('hidden');
        warning.textContent = `⚠️ Worker "${worker ? worker.name : ''}" has not linked Telegram yet. Give them their key: ${worker ? worker.key : ''}`;
      } else {
  ...
  window.closeSendMsgModal = function() {
    ...
    activeMsgWorkerId = null;
  };

  window.submitSendMsg = async function() {
    if (!activeMsgWorkerId) return;
    const input = document.getElementById('send-msg-input');
    ...
    try {
      const res = await fetch(`${API_BASE}/workers/${activeMsgWorkerId}/message`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const data = await res.json();
      if (data.success) {
        showToast('✓ Message delivered to Telegram!', 'success');
        closeSendMsgModal();
      } else {
        showToast(data.error || 'Failed to deliver message', 'danger');
      }
    } catch (e) {
      showToast('Delivery error: ' + e.message, 'danger');
    } finally {
      if (btn) {
        btn.disabled = false;
